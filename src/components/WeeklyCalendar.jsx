...
const WeeklyCalendar = ({ subscriptions }) => {
  ...
  const renderWeek = () => {
    ...
    selectedDayRef.current.innerText = currentDate
      .getDate()
      .toString()
      .padStart(2, "0");
  ...
  const buildTimeGrid = () => {
    ...
    Array.from(gridWrapper.querySelectorAll(".event")).forEach((el) =>
      el.remove(),
    );
  };
  ...
  useEffect(() => {
    const fetchEvents = async () => {
      try {
        const fromAppSnapshot = await getDocs(collection(db, "eventsFromApp"));
        const fromAppEvents = fromAppSnapshot.docs.map((doc) => doc.data());

        const legacySnapshot = await getDocs(collection(db, "events"));
        const legacyEvents = legacySnapshot.docs
          .map((doc) => {
            const data = doc.data();
            if (!data.date || isNaN(new Date(data.date))) return null; // ✅ Skip invalid entries
        ...
        setEvents([...fromAppEvents, ...legacyEvents]); // ✅ Merge both sources
      } catch (error) {
        ...
        <div className="calendar-header">
          <div className="date-nav">
            <div className="top-arrows">
              ...
              <div className="selected-day" ref={selectedDayRef}>
                01
              </div>
              <span onClick={() => changeWeek(1)}>&#x276F;</span>
            </div>
            <div className="bottom-arrows">
              ...
              <div id="monthYear" ref={monthYearRef}>
                MAR 25
              </div>
              <span onClick={() => changeMonth(1)}>&#x276F;</span>
            </div>
